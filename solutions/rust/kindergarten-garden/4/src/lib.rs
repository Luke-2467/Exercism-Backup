...
pub fn plants(diagram: &str, student: &str) -> Vec<&'static str> {
    ...
    for row in rows {
        ...
        if let Some(kid_number_real) = kid_number {
            ...
            for plant in row_vec.iter().skip((kid_number_real_usize*2)).take(1 + 1) {
                plants_result.push(plants.get(plant).expect("No plant match, add plant to plants hashtable"));
            }
        }
    }
