#!/usr/bin/env bash
declare -A number_scores=( ["ones"]="1" ["twos"]="2" ["threes"]="3" ["fours"]="4" ["fives"]="5" ["sixes"]="6" )

category="$1"; shift; rolls=("$@"); number_score="${number_scores[$category]}"; roll_summary=(); choice_score=0; max_num_rolls=0

for roll in "${rolls[@]}"; do
    current_roll_amount="${roll_summary[$roll]}"
    roll_summary["$roll"]=$((roll_summary[roll]+1))
