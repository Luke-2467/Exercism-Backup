...
#each full cycle adds one character to every rail except the top and bottom which each only get one
...
#if the number of cycles is even then the next rail to receive a character is the top
...
#if the number of cycles is odd then the next rail to receive a character is the bottom.
#also since bash always rounds down, an odd number of cycles means the top rail missed out on a character above, so add that here
else
...
#split characters across rails according to the numbers found above
declare -a rails
current_character=0
for ((rail_num=1;rail_num<=number_of_rails;rail_num++)); do
...
#finally zigzag along the rails to find the decoded message
decoded_message=""
rail_movement="+"
current_rail=1
...
#put all rails together and output
for (( rail_num=1 ; rail_num <= number_of_rails ; rail_num++ )); do
